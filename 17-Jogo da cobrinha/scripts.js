...
// let headImg = new Image();
// headImg.src = "cobra.png"; // coloque o nome da sua imagem aqui
...
function criarBG(){
    ...
    context.fillRect(0, 0, 64*box, 64*box); //desenha o retângulo usando x e y e a largura e altura setadas
}
...
function iniciarJogo(){    
    ...
    if(snake[0].x > 37*box && direction == "right") snake[0].x = 0;
    if(snake[0].x < 0 && direction == 'left') snake[0].x = 37 * box;
    if(snake[0].y > 37*box && direction == "down") snake[0].y = 0;
    if(snake[0].y < 0 && direction == 'up') snake[0].y = 37 * box;
    ...
    if(snakeX != food.x || snakeY != food.y){
        ...
        food.x = Math.floor(Math.random() * 37 +1) * box;
        food.y = Math.floor(Math.random() * 37 +1) * box;
    }
...
function criarBG() {
    // Fundo em tons suaves
    const gradiente = context.createLinearGradient(0, 0, 1000, 1000);
    gradiente.addColorStop(0, "#94c200");
    gradiente.addColorStop(1, "#94c200");
    context.fillStyle = gradiente;
    context.fillRect(0, 0, 68*box, 68*box); 
}
